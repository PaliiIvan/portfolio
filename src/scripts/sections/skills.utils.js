...
export function animateBuildingBlocks(nodes, height, onEndCallback) {
    ...
    loop:
    for (let xIteration = 0; xIteration < Math.ceil(nodesBig.size() / maxInColBig); xIteration++) {
        for (let yIteration = 0; yIteration < maxInColBig - 1; yIteration++) {
            const recPosition = { x: (xIteration * 260) + 10, y: (yIteration * 260) + 10 };
            coordBig.push(recPosition);

            iterator++;

            if (iterator === nodesBig.size())
                break loop;
        }
    }
    ...
    loop:
    for (let xIteration = 0; xIteration < (Math.ceil(nodesMid.size() / maxInColSmall) * 2); xIteration++) {
        for (let yIteration = 0; yIteration < maxInColSmall - 1; yIteration++) {

            const recPosition1 = { x: (lastPosition.x) + (xIteration * 130), y: lastPosition.y + 250 + (yIteration * 130) + 10 };
            const recPosition2 = { x: lastPosition.x + ((xIteration + 1) * 130), y: lastPosition.y + 250 + (yIteration * 130) + 10 };
            coordSmall.push(recPosition1);
            coordSmall.push(recPosition2);

            iterator++;
            if (iterator >= nodesMid.size() / 2)
                break loop;
        }
    }
    ...
    nodesBig
        ...
        .attr('transform', (d, i) => `translate(${coordBig[i].x}, ${coordBig[i].y})`)
        .each((dat, i) => { dat.tx = coordBig[i].x, dat.ty = coordBig[i].y });

    nodesBig.selectAll('rect')
    ...
    nodesMid
        .transition(buildingTransition)
        .attr('transform', (d, i) => `translate(${coordSmall[i].x}, ${coordSmall[i].y})`)
        .each((dat, i) => { dat.tx = coordSmall[i].x, dat.ty = coordSmall[i].y });


    nodesMid.selectAll('rect')
...
export async function createCrossIcon(node, onClick) {
    if (prevNode.lineContainer) {
        ...
        await prevNode.lineRight.transition()
            .duration(500)
            .attr('x1', '90%')
            .attr('x2', '90%')
            .end().catch(() => {
    ...
    let selectionArea = lineContainer.append('rect')
        .attr('x', '82.5%')
        .attr('y', '2.5%')
        .attr('width', '15%')
        .attr('height', '15%')
    ...
    lineRight.transition(crossTransition)
        .attr('x1', '85%')
        .attr('x2', '95%')
        .end().catch(() => {
...
export function openSkillDescription(selectedNode, svg, nodes) {
    ...
    const rec = groupForRect.append('rect')
        ...
        .end()
        .then(() => {
            const skillDescContainer = groupForRect.append('foreignObject')
            ...
            subItems.forEach((x, i) => {
                gsap.from(subItems[i], { y: -100, opacity: 0, duration: 0.5 + (i * 0.4) });
            });
...
export function closeSkillDescription() {
    ...
    const currentPosition = { x: +descRect.node().getAttribute('x'), y: +descRect.node().getAttribute('y') }
    const mainHtmlContainer = document.querySelector('.main__draw__container .skill-description-container');
